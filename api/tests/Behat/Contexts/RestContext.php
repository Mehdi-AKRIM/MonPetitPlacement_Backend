...
use App\Tests\Manager\ReferencesManager;
...
final class RestContext extends ApiTestCase implements Context
{
    ...
    public function iSendARequestTo(string $method, string $path): void
    {
        ...
        if(isset($options['body'])) {
            if(str_contains($options['body'],"{User")) {
                $user = $this->get_string_between($options['body'],'"{','}"');
                $referencedValue = ReferencesManager::getReference($user);
                $options['body'] = str_replace('{'.$user.'}',"/users/".$referencedValue,$options['body']);
            }
        ...
        if($entity !== "") {
            // replace by value
            $referencedValue= ReferencesManager::getReference($entity);
